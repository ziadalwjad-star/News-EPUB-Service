...
- unrecognisable common raster image bytes; during publication, recognised safe raster payloads with a mismatched declaration have the OPF media type repaired and are then re-validated
...
For GIF, JPEG, PNG and WebP resources, the validator identifies the actual payload signature. The build invokes `--repair-raster-media-types`: recognised safe mismatches are corrected in the OPF manifest, the archive is atomically rewritten in staging, and the rewritten EPUB is validated again before EPUBCheck/publication. Manual validation without the repair flag reports mismatches as `raster_media_type_warnings`; arbitrary or unrecognisable bytes remain a hard failure.
